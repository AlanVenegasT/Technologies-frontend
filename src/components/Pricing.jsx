...
export default function Pricing  ()  {
    ...
    const mostrarCard =(e,estado)=>{
        e.preventDefault();
        //console.log(estado)
            if(tabs[0].name === estado.name ||  tabs[0].name === estado ){
                setTabs( [
                    { name: 'My Account', href: '#', current: true },
                    { name: 'Company', href: '#', current: false },
                    { name: 'Team Members', href: '#', current: false },
                  ...
                  if(tabs[1].name === estado.name || tabs[1].name === estado){
                    setTabs( [
                ...
                if(tabs[2].name === estado.name || tabs[2].name === estado){
                    setTabs( [
                        { name: 'My Account', href: '#', current: false },
                        { name: 'Company', href: '#', current: false },
                ...
                if(tabs[3].name === estado.name || tabs[3].name === estado){
                    setTabs( [
                        { name: 'My Account', href: '#', current: false },
                        { name: 'Company', href: '#', current: false },
                        { name: 'Team Members', href: '#', current: false },
    ...
    console.log(date)
  return (
    <div className='bg-[#1F2023] lg:pt-32  ' id='web' > 
    <div className='mx-auto max-w-6xl  '>
        ...
        <select
          ...
          onChange = {e=>mostrarCard(e,e.target.value)} > 
          {tabs[0] && tabs.map((tab) => (
            <option key={tab.name} 
